...
(function () {
  ...
  function isUserMediaUrl(url) {
    if (!url) return false;
    var raw = String(url);
    for (var i = 0; i < USER_MEDIA_MARKERS.length; i += 1) {
      if (raw.includes(USER_MEDIA_MARKERS[i])) return true;
    }
    return false;
  }
  ...
  function isGeneratedImageUrl(url) {
    if (!url || !isHttpUrl(url)) return false;
    for (var i = 0; i < GEN_IMAGE_MARKERS.length; i += 1) {
      if (url.includes(GEN_IMAGE_MARKERS[i])) return true;
    }
    return false;
  }
  ...
  function hasGenScope(el) {
    ...
    for (
      var i = 0;
      i < 14 && node && node !== document.body && node !== document.documentElement;
      i += 1
    ) {
      var sig = scopeSignature(node);
      if (sig) {
        for (var g = 0; g < GEN_SCOPE_HINTS.length; g += 1) {
          if (sig.includes(GEN_SCOPE_HINTS[g])) return true;
        }
      }
      node = node.parentElement;
    }
  ...
  function scopeOf(el) {
    ...
    for (
      ...
      var sig = scopeSignature(node);
      if (sig) {
        for (var u = 0; u < USER_SCOPE_HINTS.length; u += 1) {
          if (sig.includes(USER_SCOPE_HINTS[u])) return 'user';
        }
        for (var b = 0; b < BOT_SCOPE_HINTS.length; b += 1) {
          if (sig.includes(BOT_SCOPE_HINTS[b])) return 'bot';
        }
      }
      node = node.parentElement;
    }
